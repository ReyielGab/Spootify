...
export default function DiscoverBlock({ text, id, data, imagesKey = 'images', dispatch }) {
  return (
    <div className="discover-block">
      ...
      <div className="discover-block__row" id={id}>
        {data.map(({ [imagesKey]: images, name, uri }) => (
          <DiscoverItem key={name} images={images} name={name} trackUri={uri}  dispatch={dispatch} />
        ))}
      </div>
    </div>
  );
